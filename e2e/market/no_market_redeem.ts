...
import { alwaysSuccessMintValidatorHash, blockchainProvider, depositScriptTxHash, depositScriptTxIdx, invalidAfter, invalidBefore, MarketCreatorNft, MinMarketLovelace, NoTokenName, PrecisionFactor, txBuilder, UsdmAssetName, UsdmUnit, wallet1, wallet1Address, wallet1Collateral, wallet1SK, wallet1Utxos, wallet1VK } from "../setup.js";
...
if (winnerShare !== 1) throw new Error("No didn't win!");
...
// YesMarketBuy
const MarketExecution = mConStr(6, [noAmount, mPubKeyAddress(wallet1VK, wallet1SK)]);

const marketUsdm = marketUtxo.output.amount.find(amt => amt.unit === UsdmUnit);
if (!marketUsdm) throw new Error("No usdm in market");
const userReduceValue = noAmount * multiplier;
...
console.log("No market redeem tx hash:", txHash);

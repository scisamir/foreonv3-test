import { deserializeDatum, IWallet, mConStr0, mConStr1, mConStr3, MeshTxBuilder, mPubKeyAddress, UTxO } from "@meshsdk/core";
...
export const NoMarketBuy = async (
  ...
  const userAddedValue = (pNo * noAmount * multiplier) / PrecisionFactor;
  ...
  // NoMarketBuy
  const MarketExecution = mConStr3([noAmount, mPubKeyAddress(walletVK, walletSK)]);
  ...
  const unsignedTx = await txBuilder
      ...
      // send user "No" tokens
      // .txOut(walletAddress, [{ unit: "lovelace", quantity: "1200000" }, { unit: tokenUnit, quantity: String(noAmount) }])
      .txOut(walletAddress, [{ unit: tokenUnit, quantity: String(noAmount) }])
